  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response1 = await fetch(
        "http://localhost:3001/hospital-data/sendEmail",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      const response2 = await fetch(
        "http://localhost:3001/rescue-team-data/sendEmail",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      const response1 = await fetch(
        "http://localhost:3001/hospital-data/sendSMS",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response1.status === 200) {
        const data1 = await response1.json();
        console.log("SMS sent successfully:", data1);
        // You can also reset the form or redirect the user to a success page here
      } else {
        console.error("SMS sending failed:", response1);
      }

      const response2 = await fetch(
        "http://localhost:3001/rescue-team-data/sendSMS",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response2.status === 200) {
        const data2 = await response2.json();
        console.log("SMS sent successfully:", data2);
        // You can also reset the form or redirect the user to a success page here
      } else {
        console.error("SMS sending failed:", response2);
      }
    } catch (error) {
    ...
    const response3 = await fetch(
      "http://localhost:3001/user-subscription/sendEmail",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className="lg:grid lg:grid-cols-2 w-[100%]">
      ...
      <div className="max-w-md mx-auto lg:mt-20 mt-10 md:w-[90%] w-[90%] ">
        <h2 className="font-bold text-3xl text-center lg:mt-10 mb-20 mt-5 ">
          <span className="text-red-600 font-extrabold font-serif animate-pulse text-5xl">
            Alert{" "}
          </span>{" "}
          Fill Now To Update
        </h2>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={handleChange}
              className={`w-full px-4 py-2 rounded-lg border focus:outline-none focus:border-blue-500 ${
                credentialError ? "border-red-500" : ""
              }`}
              required
            />
            {credentialError && (
              <p className="text-red-500 text-sm mt-2">
                Credential ID is incorrect. Please try again.
              </p>
            )}
          </div>
